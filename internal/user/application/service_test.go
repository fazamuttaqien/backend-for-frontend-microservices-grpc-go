...
func (r *repo) GetByEmail(_ context.Context, email string) (*domain.User, error) {
...
func TestCreateHashesPassword(t *testing.T) {
	...
	u, e := s.Create(context.Background(), "Faza", "FAZA@EXAMPLE.COM", "secret")
	if e != nil {
		t.Fatal(e)
	}
...
func TestAuthenticate(t *testing.T) {
	r := &repo{}
	s := application.NewUserService(r)
	u, _ := s.Create(context.Background(), "Faza", "faza@example.com", "secret")
	if _, e := s.Authenticate(context.Background(), "FAZA@EXAMPLE.COM", "secret"); e != nil {
		t.Fatal(e)
	}
	if _, e := s.Authenticate(context.Background(), u.Email, "wrong"); e != domain.ErrInvalidCredentials {
		t.Fatal("wrong password should fail with invalid credentials")
	}
	if _, e := s.Authenticate(context.Background(), "missing@example.com", "secret"); e != domain.ErrInvalidCredentials {
		t.Fatal("unknown email should fail with invalid credentials")
	}
}
func TestGetAndUpdate(t *testing.T) {
	r := &repo{}
	s := application.NewUserService(r)
	u, e := s.Create(context.Background(), "Old", "old@example.com", "secret")
	if e != nil {
		t.Fatal(e)
	}
	got, e := s.Get(context.Background(), u.ID)
	if e != nil || got.ID != u.ID {
		t.Fatal(e)
	}
	updated, e := s.Update(context.Background(), u.ID, "New", "new@example.com", "")
	if e != nil {
		t.Fatal(e)
	}
