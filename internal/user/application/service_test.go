...
func TestCreateHashesPassword(t *testing.T){r:=&repo{};s:=application.NewUserService(r);u,err:=s.Create(context.Background(),"Faza","FAZA@EXAMPLE.COM","secret");if err!=nil{t.Fatal(err)};if u.PasswordHash=="secret"||u.PasswordHash==""{t.Fatal("password was not hashed")};if u.Email!="faza@example.com"{t.Fatalf("email=%q",u.Email)}}
func TestGetAndUpdate(t *testing.T){r:=&repo{};s:=application.NewUserService(r);u,err:=s.Create(context.Background(),"Old","old@example.com","secret");if err!=nil{t.Fatal(err)};got,err:=s.Get(context.Background(),u.ID);if err!=nil||got.ID!=u.ID{t.Fatal(err)};updated,err:=s.Update(context.Background(),u.ID,"New","new@example.com","");if err!=nil{t.Fatal(err)};if updated.Name!="New"||updated.Email!="new@example.com"{t.Fatal("update failed")};if updated.PasswordHash!=u.PasswordHash{t.Fatal("password hash changed unexpectedly")}}
