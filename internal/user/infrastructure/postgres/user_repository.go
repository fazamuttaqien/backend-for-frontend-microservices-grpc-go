...
func (r *UserRepository) Create(ctx context.Context, u *domain.User) error {
	...
	if errors.As(err, &pe) && pe.Code == "23505" { return domain.ErrEmailAlreadyExists }
	return err
}
...
func (r *UserRepository) GetByEmail(ctx context.Context, email string) (*domain.User, error) {
	u := &domain.User{}
	err := r.db.QueryRow(ctx, `SELECT id,name,email,password_hash,created_at,updated_at FROM users WHERE email=$1`, email).Scan(&u.ID, &u.Name, &u.Email, &u.PasswordHash, &u.CreatedAt, &u.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) { return nil, domain.ErrUserNotFound }
	if err != nil { return nil, err }
	return u, nil
}
func (r *UserRepository) Update(ctx context.Context, u *domain.User) error {
	...
	if tag.RowsAffected() == 0 { return domain.ErrUserNotFound }
	return nil
}
