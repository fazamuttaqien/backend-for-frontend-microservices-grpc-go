...
func NewHandler(app *application.UserService, jwt *auth.JWT) *Handler {
...
func response(u *domain.User) *userv1.User {
	return &userv1.User{Id: u.ID, Name: u.Name, Email: u.Email, CreatedAt: u.CreatedAt.UTC().Format(timeFormat), UpdatedAt: u.UpdatedAt.UTC().Format(timeFormat)}
}
...
func (h *Handler) CreateUser(ctx context.Context, in *userv1.CreateUserRequest) (*userv1.CreateUserResponse, error) {
	...
	u, e := h.app.Create(ctx, in.Name, in.Email, in.Password)
	if e != nil {
		return nil, mapErr(e)
	}
	return &userv1.CreateUserResponse{User: response(u)}, nil
}
func (h *Handler) Register(ctx context.Context, in *userv1.CreateUserRequest) (*userv1.CreateUserResponse, error) {
	return h.CreateUser(ctx, in)
}
func (h *Handler) Login(ctx context.Context, in *userv1.LoginRequest) (*userv1.LoginResponse, error) {
	if in == nil {
		return nil, status.Error(codes.InvalidArgument, "request is required")
	}
	u, e := h.app.Authenticate(ctx, in.Email, in.Password)
	if e != nil {
		return nil, mapErr(e)
	}
	token, e := h.jwt.Issue(u.ID, u.Email)
	if e != nil {
		return nil, status.Error(codes.Internal, "internal server error")
	}
	claims, e := h.jwt.Parse(token)
	if e != nil {
		return nil, status.Error(codes.Internal, "internal server error")
	}
	return &userv1.LoginResponse{AccessToken: token, ExpiresAt: claims.Expires, User: response(u)}, nil
}
func (h *Handler) GetUser(ctx context.Context, in *userv1.GetUserRequest) (*userv1.GetUserResponse, error) {
	...
	if !ok || claims.Subject != in.Id {
		return nil, status.Error(codes.PermissionDenied, "forbidden")
	}
	u, e := h.app.Get(ctx, in.Id)
	if e != nil {
		return nil, mapErr(e)
	}
...
func (h *Handler) UpdateUser(ctx context.Context, in *userv1.UpdateUserRequest) (*userv1.UpdateUserResponse, error) {
	...
	u, e := h.app.Update(ctx, in.Id, in.Name, in.Email, in.Password)
	if e != nil {
		return nil, mapErr(e)
	}
