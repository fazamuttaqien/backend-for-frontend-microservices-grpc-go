...
func NewUser(id, name, email, passwordHash string, now time.Time) (*User, error) {
	...
	if id == "" || name == "" || email == "" || passwordHash == "" || len(name) > 100 || len(email) > 254 { return nil, ErrInvalidUser }
...
func (u *User) Update(name, email, passwordHash string, now time.Time) error {
	name = strings.TrimSpace(name); email = strings.ToLower(strings.TrimSpace(email))
	if name == "" || email == "" || len(name) > 100 || len(email) > 254 { return ErrInvalidUser }
	u.Name=name; u.Email=email; if passwordHash != "" { u.PasswordHash=passwordHash }; u.UpdatedAt=now; return nil
}
