...
func NewJWT(secret, issuer string, ttl time.Duration) (*JWT, error) {
	...
	return &JWT{[]byte(secret), issuer, ttl, time.Now}, nil
}
func (j *JWT) Issue(userID, email string) (string, error) {
	...
	n := j.now().UTC()
	c := Claims{Subject: userID, Email: email, Issuer: j.issuer, Issued: n.Unix(), Expires: n.Add(j.ttl).Unix()}
	h := b64([]byte(`{"alg":"HS256","typ":"JWT"}`))
	pbytes, e := json.Marshal(c)
	if e != nil {
		return "", e
	}
	u := h + "." + b64(pbytes)
	return u + "." + j.sign(u), nil
}
func (j *JWT) Parse(token string) (*Claims, error) {
	...
	sig := j.sign(p[0] + "." + p[1])
	if subtle.ConstantTimeCompare([]byte(sig), []byte(p[2])) != 1 {
		return nil, ErrInvalidToken
	}
	pb, e := base64.RawURLEncoding.DecodeString(p[1])
	if e != nil {
		return nil, ErrInvalidToken
	}
	var c Claims
	if json.Unmarshal(pb, &c) != nil || c.Subject == "" || c.Email == "" || c.Issuer != j.issuer {
		return nil, ErrInvalidToken
	}
	if c.Expires <= j.now().Unix() {
		return nil, ErrExpiredToken
	}
	if c.Issued <= 0 || c.Issued > j.now().Add(time.Minute).Unix() {
		return nil, ErrInvalidToken
	}
	return &c, nil
}
func (j *JWT) sign(v string) string {
