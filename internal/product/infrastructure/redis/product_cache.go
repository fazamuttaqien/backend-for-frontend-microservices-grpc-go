...
func NewProductCache(address, password string, db int, timeout time.Duration) *ProductCache {
	return &ProductCache{address: address, password: password, db: db, timeout: timeout}
}
...
func (c *ProductCache) Set(ctx context.Context, key string, value []byte) error {
	...
	if err := writeCommand(conn, "SET", key, string(value), "EX", strconv.FormatInt(int64(c.timeout/time.Second), 10)); err != nil {
		return err
	}
