...
func (r *repo) Create(_ context.Context, p *domain.Product) error {
	if r.products == nil { r.products = map[string]*domain.Product{} }
	r.products[p.ID] = p
	return nil
}
...
func (c *cache) Get(_ context.Context, key string) ([]byte, error) {
	c.gets++
	if c.getErr != nil { return nil, c.getErr }
	v, ok := c.values[key]
	if !ok { return nil, errors.New("cache miss") }
	return v, nil
}
...
func (c *cache) Delete(_ context.Context, key string) error {
	c.deletes++
	if c.deleteErr != nil { return c.deleteErr }
	delete(c.values, key)
	return nil
}
...
func TestCreate(t *testing.T) {
	...
	if p.Name != "Keyboard" || p.Price != "99.90" || p.Stock != 10 { t.Fatal("create failed") }
}
...
func TestGetListAndUpdate(t *testing.T) {
	...
	if err != nil || got.ID != p.ID { t.Fatal(err) }
	items, total, err := s.List(context.Background(), 1, 20)
	if err != nil || total != 1 || len(items) != 1 { t.Fatal("list failed") }
	updated, err := s.Update(context.Background(), p.ID, "New", "Updated", "20", 5)
	if err != nil { t.Fatal(err) }
	if updated.Name != "New" || updated.Price != "20" || updated.Stock != 5 { t.Fatal("update failed") }
}
...
func TestGetCacheHitSkipsDatabase(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if got.Name != "Keyboard" || r.gets != 0 { t.Fatalf("cache hit failed: got=%+v dbGets=%d", got, r.gets) }
}
...
func TestGetCacheMissReadsDatabaseAndPopulatesCache(t *testing.T) {
	...
	got, err := s.Get(context.Background(), p.ID)
	if err != nil || got.ID != p.ID { t.Fatal(err) }
	if r.gets != 1 || c.gets != 1 || c.sets != 1 { t.Fatalf("unexpected cache-aside calls: db=%d get=%d set=%d", r.gets, c.gets, c.sets) }
}
...
func TestGetFallsBackWhenCacheUnavailable(t *testing.T) {
	...
	got, err := s.Get(context.Background(), p.ID)
	if err != nil || got.ID != p.ID { t.Fatal(err) }
	if r.gets != 1 { t.Fatalf("expected database fallback, got %d reads", r.gets) }
}
...
func TestUpdateInvalidatesCacheAfterDatabaseWrite(t *testing.T) {
	...
	s := application.NewProductService(r, c)
	if _, err := s.Update(context.Background(), p.ID, "New", "", "20", 2); err != nil { t.Fatal(err) }
	if c.deletes != 1 { t.Fatalf("expected one invalidation, got %d", c.deletes) }
	if _, ok := c.values["product:v1:p1"]; ok { t.Fatal("cache entry was not invalidated") }
}

func TestInvalidProduct(t *testing.T) {
	r := &repo{}
	s := application.NewProductService(r)
	if _, err := s.Create(context.Background(), "", "", "10", 0); err != domain.ErrInvalidProduct { t.Fatal("expected invalid product") }
	if _, err := s.Create(context.Background(), "x", "", "10", -1); err != domain.ErrInvalidProduct { t.Fatal("expected invalid stock") }
}
...
func TestCanceledContextDoesNotCallRepository(t *testing.T) {
	...
	r := &repo{}
	s := application.NewProductService(r)
	if _, err := s.Create(ctx, "Keyboard", "Mechanical", "99.90", 10); !errors.Is(err, context.Canceled) { t.Fatalf("Create: %v", err) }
	if _, err := s.Get(ctx, "missing"); !errors.Is(err, context.Canceled) { t.Fatalf("Get: %v", err) }
	if _, _, err := s.List(ctx, 1, 20); !errors.Is(err, context.Canceled) { t.Fatalf("List: %v", err) }
	if _, err := s.Update(ctx, "missing", "x", "", "10", 1); !errors.Is(err, context.Canceled) { t.Fatalf("Update: %v", err) }
}
