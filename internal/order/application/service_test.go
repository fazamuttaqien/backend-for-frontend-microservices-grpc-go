...
func (m *mockRepo) GetByID(_ context.Context, _ string) (*domain.Order, error) {
	if m.err != nil {
		return nil, m.err
	}
	if len(m.orders) == 0 {
...
func (m *mockRepo) List(_ context.Context, _, _ int) ([]*domain.Order, int, error) {
	return m.orders, len(m.orders), m.err
}
...
func TestCreate(t *testing.T) {
	...
	if o.Total != "25.0000" || o.Status != "PENDING" {
		t.Fatalf("unexpected order: %+v", o)
	}
	if len(o.Items) != 1 || o.Items[0].Price != "12.5000" || o.Items[0].Total != "25.0000" {
		t.Fatalf("unexpected item: %+v", o.Items)
	}
}
...
func TestListDefaultsAndGet(t *testing.T) {
	...
	if err != nil || total != 1 || len(orders) != 1 {
		t.Fatalf("unexpected list result: %v %d %d", err, total, len(orders))
	}
	o, err := svc.Get(context.Background(), "o-1")
	if err != nil || o.ID != "o-1" {
		t.Fatalf("unexpected get result: %+v %v", o, err)
	}
}
