...
func TestOrderCreatedEnvelope(t *testing.T) {
	...
	if event.Type != OrderCreatedType || event.Version != OrderCreatedVersion { t.Fatalf("unexpected event metadata: %+v", event) }
	if event.EventID != "event-1" || event.Data.OrderID != order.ID { t.Fatalf("unexpected event data: %+v", event.Data) }
	body, err := event.Marshal()
	if err != nil || len(body) == 0 { t.Fatalf("expected JSON event, got %v", err) }
}
