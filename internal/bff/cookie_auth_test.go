...
func TestCookieAuthLoginSetsHttpOnlyCookieAndHidesToken(t *testing.T) {
	...
	cookie := cookies[0]
	if cookie.Name != "access_token" || !cookie.HttpOnly || cookie.Path != "/" || cookie.SameSite != http.SameSiteLaxMode {
		t.Fatalf("unexpected cookie: %+v", cookie)
	}
	if cookie.Value == "" {
		t.Fatal("cookie token is empty")
	}
...
func TestCookieAuthLogoutDeletesCookie(t *testing.T) {
	...
	cookies := w.Result().Cookies()
	if len(cookies) != 1 || cookies[0].Name != "access_token" || cookies[0].MaxAge != -1 || cookies[0].Path != "/" {
...
func TestCookieAuthRejectsCrossOriginCookieWrite(t *testing.T) {
	...
	h := CookieAuthHandler(s.Handler(), CookieConfig{Name: "access_token", Path: "/", AllowedOrigins: []string{"http://localhost:3000"}})
	r := httptest.NewRequest(http.MethodPost, "/api/v1/auth/logout", nil)
...
func TestCORSMiddlewareAllowsConfiguredOriginWithCredentials(t *testing.T) {
	...
	h := CORSMiddleware([]string{"http://localhost:3000"}, true)(next)
	...
	if w.Header().Get("Access-Control-Allow-Origin") != "http://localhost:3000" {
		t.Fatal("origin was not allowed")
	}
	if w.Header().Get("Access-Control-Allow-Credentials") != "true" {
		t.Fatal("credentials were not enabled")
	}
}
