...
func (c *authUserClient) Register(context.Context, string, string, string) (*userv1.User, error) {
	return c.registered, nil
}
func (c *authUserClient) Login(context.Context, string, string) (*userv1.LoginResponse, error) {
...
func TestRegisterSuccess(t *testing.T) {
	s := newAuthTestServer(t, &authUserClient{registered: &userv1.User{Id: "u1", Name: "Faza", Email: "faza@example.com"}})
...
func TestLoginSuccessAndIssuedTokenWorks(t *testing.T) {
	...
	claims, err := s.jwt.Parse(got.AccessToken)
	if err != nil || claims.Subject != "u1" {
		t.Fatalf("token parse error: %v", err)
	}
}
func TestLoginInvalidCredentialsIsRejected(t *testing.T) {
	s := newAuthTestServer(t, &authUserClient{loginErr: status.Error(codes.Unauthenticated, "invalid credentials")})
	...
	if strings.Contains(w.Body.String(), "invalid credentials") {
		t.Fatalf("internal detail exposed: %s", w.Body.String())
	}
}
...
func TestMeRejectsExpiredToken(t *testing.T) {
	jwt, err := auth.NewJWT(strings.Repeat("s", 32), "user-service", time.Nanosecond)
	if err != nil {
		t.Fatal(err)
	}
	s := NewServer(jwt, Clients{User: &authUserClient{}})
	token, err := jwt.Issue("u1", "faza@example.com")
	if err != nil {
		t.Fatal(err)
	}
