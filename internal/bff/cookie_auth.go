...
func newAuthCaptureWriter() *authCaptureWriter {
	return &authCaptureWriter{header: make(http.Header), statusCode: http.StatusOK}
}
...
func (w *authCaptureWriter) Write(p []byte) (int, error) {
	if !w.wroteHeader {
...
func (w *authCaptureWriter) writeTo(dst http.ResponseWriter) {
	for key, values := range w.header {
		dst.Header()[key] = values
	}
	// The login response is rewritten after removing access_token, so an old
	// Content-Length would be incorrect. Do not copy it from the captured response.
