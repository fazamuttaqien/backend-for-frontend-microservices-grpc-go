...
func Load() Config {
	return Config{
		...
		AuthCookieName:        env("AUTH_COOKIE_NAME", "access_token"),
		AuthCookiePath:        env("AUTH_COOKIE_PATH", "/"),
		AuthCookieSecure:      envBool("AUTH_COOKIE_SECURE", false),
		AuthCookieSameSite:    strings.ToLower(env("AUTH_COOKIE_SAMESITE", "lax")),
		CORSAllowedOrigins:    envCSV("CORS_ALLOWED_ORIGINS", "http://localhost:5173"),
		CORSCredentials:       envBool("CORS_ALLOW_CREDENTIALS", true),
