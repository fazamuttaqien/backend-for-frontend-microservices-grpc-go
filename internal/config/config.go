...
func envBool(k string, def bool) bool { if v:=os.Getenv(k); v!="" { if n,err:=strconv.ParseBool(v); err==nil { return n } }; return def }
func envCSV(k, def string) []string { parts:=strings.Split(env(k,def),","); out:=make([]string,0,len(parts)); for _,part:=range parts { if value:=strings.TrimSpace(part); value!="" { out=append(out,value) } }; return out }
