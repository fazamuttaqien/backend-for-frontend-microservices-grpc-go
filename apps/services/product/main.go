...
func main() {
	...
	if cfg.ProductDatabaseURL == "" || cfg.JWTSecret == "" { logger.Error("required configuration is missing"); return }
	...
	db, err := database.OpenPostgres(startup, cfg.ProductDatabaseURL); if err != nil { logger.Error("initialize product database", "error", err); return }; defer db.Close()
	if err = db.Ping(startup); err != nil { logger.Error("ping product database", "error", err); return }
	telemetry := observability.Setup(ctx, "product-service"); defer telemetry.Shutdown(context.Background())
	jwt, err := auth.NewJWT(cfg.JWTSecret, cfg.JWTIssuer, cfg.JWTTTL); if err != nil { logger.Error("initialize JWT", "error", err); return }
	repo := postgres.NewProductRepository(db); cache := productredis.NewProductCache(cfg.RedisAddress, cfg.RedisPassword, cfg.RedisDB, cfg.RedisTimeout, cfg.ProductCacheTTL); app := application.NewProductService(repo, cache); h := productgrpc.NewHandler(app)
	lis, err := net.Listen("tcp", ":"+cfg.ProductGRPCPort); if err != nil { logger.Error("listen product service", "error", err); return }
	...
	productv1.RegisterProductServiceServer(s, h); hs := health.Register(s, "product-service")
	logger.Info("product service listening", "port", cfg.ProductGRPCPort)
	go func(){ if err := s.Serve(lis); err != nil && !errors.Is(err, grpc.ErrServerStopped) { logger.Error("serve product service", "error", err); cancel() } }()
