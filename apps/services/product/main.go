...
func main() {
	...
	db, err := pgxpool.New(context.Background(), cfg.ProductDatabaseURL); if err != nil { log.Fatal(err) }
	defer db.Close()
	...
	lis, err := net.Listen("tcp", ":"+cfg.ProductGRPCPort); if err != nil { log.Fatal(err) }
	s := grpc.NewServer()
